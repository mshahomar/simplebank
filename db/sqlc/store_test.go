...
func TestTransfer(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		// check if transfer record is really created in database
		_, err = store.GetTransfer(context.Background(), transfer.ID)
		require.NoError(t, err)

		// check each of the account entries
		...
		// check if from_entry record is really created in database
		...
		// check if to_entry record is really created in database
		_, err = store.GetEntry(context.Background(), toEntry.ID)
		require.NoError(t, err)

		// check from accounts (where money is going out)
		fromAcc := result.FromAccount
		require.NotEmpty(t, fromAcc)
		require.Equal(t, account1.ID, fromAcc.ID)

		// check to accounts (where money is coming in)
